...
func _process(delta: float) -> void:
	...
	_update_ghost()
	info_label.text = "%d pieces  ·  %d spawns" % [level.data.pieces.size(), level.spawn_points.size()]
...
func _make_ghost() -> void:
	...
	if info.has("scene"):
		...
		box.size = info.size
		ghost.mesh = box
		ghost.position.y = info.size.y / 2
		var mat := StandardMaterial3D.new()
		mat.albedo_color = Color(1, 1, 0.4, 0.4) if type == "spawn" else Color(0.5, 0.9, 1, 0.4)
...
func _build_ui() -> void:
	...
	new_btn.pressed.connect(func():
		level.build({"name": "Untitled", "pieces": [{"t": "platform_large", "p": [0, 0, 0], "r": 0}, {"t": "spawn", "p": [0, 1, 0], "r": 0}]}, true)
	...
	play.pressed.connect(func():
		if level.spawn_points.is_empty():
			set_status("Add at least one spawn point first")
			return
...
func _save() -> void:
	if level.spawn_points.is_empty():
		set_status("Add at least one spawn point before saving")
		return
